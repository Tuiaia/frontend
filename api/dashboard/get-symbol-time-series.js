 ...
 */
async function getSymbolTimeSeries(inputSymbol, startDate, endDate) {
    try {
...
(async function() {
    try {
        let retorno = await getSymbolTimeSeries('IBM', '2023-05-12', '2023-05-12');
        console.log(retorno['Time Series (Daily)']);
    } catch (error) {
        console.error(error);
    }
})();